...
try:
    from experiment_runner_fixed import stream_qft_experiment, stream_advantage_experiment
    EXPERIMENT_RUNNER_AVAILABLE = True
    EXPERIMENT_RUNNER_TYPE = 'fixed'
except ImportError:
    try:
        from experiment_runner import QFTRunner, QuantumAdvantageRunner, generate_experiment_stream
        EXPERIMENT_RUNNER_AVAILABLE = True
        EXPERIMENT_RUNNER_TYPE = 'original'
    except ImportError:
        EXPERIMENT_RUNNER_AVAILABLE = False
        EXPERIMENT_RUNNER_TYPE = None
...
def stream_qft():
    ...
    # Use fixed runner if available
    if EXPERIMENT_RUNNER_TYPE == 'fixed':
        n_qubits = request.args.get('n_qubits', None)
        if n_qubits:
            n_qubits = int(n_qubits)
        
        return Response(
            stream_qft_experiment(STATE.db_path, n_qubits),
...
def stream_advantage():
    ...
    if EXPERIMENT_RUNNER_TYPE == 'fixed':
        return Response(
            stream_advantage_experiment(STATE.db_path),
            mimetype='text/event-stream',
            headers={
                'Cache-Control': 'no-cache',
                'X-Accel-Buffering': 'no'
